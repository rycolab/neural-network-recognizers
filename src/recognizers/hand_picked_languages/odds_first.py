...
class OddsFirst(WeightedLanguage):
    ...
    def with_length_range(self, length_range):
        return LengthRestrictedOddsFirst(self, length_range)

class LengthRestrictedOddsFirst(LengthRestrictedWeightedLanguage):
    ...
    def _is_positive(self, s):
        ...
        if i is None:
            return False
        return s[i+1:] == self._odds_first(s[:i])
    ...
    def _w_to_string(self, w):
        marker = self.parent.num_symbols
        return (*w, marker, *self._odds_first(w))
    ...
    def _w_to_next_symbols(self, w):
        ...
        for _ in range(len(w) + 1):
            result.append(r)
        for a in self._odds_first(w):
            result.append([a])
        result.append([ReservedSymbol.EOS])
        return result

    def _odds_first(self, w):
        return w[::2] + w[1::2]
